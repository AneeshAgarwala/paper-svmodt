...
author:  
  - name: Aneesh Agarwal
    ...
    email:  aneesh.agarwal@monash.edu
  - name: Jack Jewson
    affiliation:
    - Monash University
    address:
...
Employing linear classifiers at internal nodes can yield decision trees that are both accurate and diverse. Because each node receives a different subset of the training data, the most suitable classification boundary may vary from node to node. For example, [@menze2011; @lemmond2010] use Linear Discriminant Analysis (LDA) to construct linear splits, though their approach is limited to binary classification. In contrast,  @Truong2009 introduced multiple logistic‑regression–based partitions (specifically $2^{K-1} - 1$ for $K$ classes) to select selects the split that best optimizes the impurity criterion.

Projection pursuit methods [@friedman2006; @kruskal1969] have also been used to identify low‑dimensional projections that expose structure in high‑dimensional data. The \CRANpkg{PPtreeExt}[@pptree2013; @Silva2021] algorithm extends this idea into a recursive partitioning framework by optimizing a class‑sensitive projection index at each node and then applying standard split rules to the resulting one‑dimensional projection. Similarly, @Bulo2014 introduce a randomized multi‑layer perceptron as a node‑level splitting function; however, determining an appropriate network complexity remained challenging, as overly flexible models risked substantial overfitting.
...
Support Vector Machine–based decision trees were first formalized by @bennet1998, who adapted Statistical Learning Theory and Structural Risk Minimization to construct binary decision trees in which each internal node is an SVM that partitions the data by an optimal hyperplane. This formulation enabled multivariate, margin-based decisions at every split. @takahashi2002 extended the idea to multiclass settings by using a recursive partitioning strategy: the root node isolates the most separable class or classes from the remainder, and the procedure recurses until each leaf contains a single class, thereby covering all classes. Subsequent studies have built upon this foundation to address scalability, optimisation, and generalisation issues. Optimal Decision Tree SVM (ODT-SVM) [@bala2011] introduced split-selection criteria based on Gini index, information gain, and scatter matrix separability to balance tree interpretability and margin-based precision. @Reynolds2019 further extended the framework by incorporating class-specific cost parameters through the assignment of weights to individual classes, improving performance in imbalanced classification settings. 

### STree
More recent approaches embed multiple SVM classifiers directly within each split to handle multi-class problems more efficiently. STree [@stree2021; @Montanana2021] is an oblique multi-class decision tree algorithm that integrates support vector machines (SVMs) to construct single margin‑based splits capable of handling multiclass problems. Unlike traditional multi-class tree methods that rely on clustering or ensembles of binary models, STree builds a single decision tree. Its central innovation lies in using SVM‑derived hyperplanes at each internal node, enabling more expressive splits than axis‑aligned trees while also being interpretable and computationally simple.

The algorithm generates a binary tree recursively (see `r knitr::asis_output(ifelse(knitr::is_latex_output(), 'Algorithm \\@ref(alg:stree)', 'Figure \\@ref(fig:alg-stree-html)'))` in the Appendix). At each node:
...
We introduce STreeR, a R implementation of the STree algorithm. As no such implementation currently exists, we reconstruct the method in R using the \CRANpkg{e1071} package [@e10712024], which interfaces with the LIBSVM C++ backend. This reconstruction provides greater transparency into the algorithm’s internal workflow and enables fair benchmarking against competing approaches. `r knitr::asis_output(ifelse(knitr::is_latex_output(), 'Algorithm \\@ref(alg:streer)', 'Figure \\@ref(fig:alg-streer-html)'))` provides a concise summary of the recursive splitting underlying the STreeR implementation.
...
```{r alg-streer-html, echo=FALSE, results='asis', eval=knitr::is_html_output(), fig.cap='STreeR: Multi-class OVR SVM-based oblique decision tree', out.width='120%'}
knitr::include_graphics("figures/algo-1.png")
```
...
r_stree <- readRDS("analysis/r_stree.rds")
py_stree <- readRDS("analysis/py_stree.rds")
time_bench <- readRDS("analysis/time-benchmark.rds")
...
# readRDS("analysis/optimised_r_stree.rds")
...
When the node contains exactly two classes ($k = 2$), a binary SVM classifier is trained (see `r knitr::asis_output(ifelse(knitr::is_latex_output(), 'Algorithm \\@ref(alg:binary-split)', 'Figure \\@ref(fig:alg-binary-split-html)'))`) using the selected feature subset $\mathcal{F}_d$. Let $\mathbf{X}_{\text{scaled}}$ denote the standardized feature matrix at that node and $\mathcal{Y}$ the corresponding class labels.
...
```{r alg-binary-split-html, echo=FALSE, results='asis', eval=knitr::is_html_output(), fig.cap='Binary Classification Split', out.width='120%'}
knitr::include_graphics("figures/algo-2.png")
```
...
```{r alg-multi-split-html, echo=FALSE, results='asis', eval=knitr::is_html_output(), fig.cap='Multiclass One-vs-Rest Split', out.width='120%'}
knitr::include_graphics("figures/algo-3.png")
```
...
knitr::include_graphics(path = "figures/tour-constant.gif")
knitr::include_graphics(path = "figures/tour-decrease.gif")
knitr::include_graphics(path = "figures/tour-random-maxfeat.gif")
```

```{r svmodt-max-features-accuracy-score-table}
# Load results
svmodt_max_features_constant <- readRDS("analysis/svmodt_mf_constant.rds")
svmodt_max_features_decrease <- readRDS("analysis/svmodt_mf_decrease.rds")
svmodt_max_features_random <- readRDS("analysis/svmodt_mf_random.rds")
py_stree <- readRDS("analysis/py_stree.rds")
...
knitr::include_graphics(path = "figures/tour-random.gif")
knitr::include_graphics(path = "figures/tour-mutual.gif")
knitr::include_graphics(path = "figures/tour-corr.gif")
```
...
# Load results
svmodt_random <- readRDS("analysis/svmodt_feature_random.rds")
svmodt_mutual <- readRDS("analysis/svmodt_feature_mutual.rds")
svmodt_cor <- readRDS("analysis/svmodt_feature_cor.rds")
py_stree <- readRDS("analysis/py_stree.rds")
...
svmodt_penalty_none <- readRDS("analysis/svmodt_penalty_none.rds")
svmodt_penalty_low <- readRDS("analysis/svmodt_penalty_low.rds")
svmodt_penalty_medium <- readRDS("analysis/svmodt_penalty_medium.rds")
svmodt_penalty_high <- readRDS("analysis/svmodt_penalty_high.rds")
py_stree <- readRDS("analysis/py_stree.rds")
...
count_tree_nodes <- function(tree) {
  splits <- function(node) {
    if (is.null(node) || isTRUE(node$is_leaf)) {
      return(0L)
    }
...
min_sample_table <- cbind(
  rbind(
    `10` = count_tree_nodes(tree_wdbc_min_10_sample),
...
min_sample_table |>
  ...
  kbl(
    col.names = c("Min Samples", "Splits", "Leaves", "Total Nodes", "Accuracy"),
    align = "ccccc",
    escape = FALSE,
...
cbind(
  rbind(
    `3` = count_tree_nodes(tree_cardio_depth_3),
  ...
  kable(
    col.names = c("Max Depth", "Splits", "Leaves", "Total Nodes", "Accuracy"),
    align = "lcccc",
    caption = paste0(
...
cbind(
  rbind(
    `0.005` = count_tree_nodes(tree_cardio_impurity_0005),
  ...
  kable(
    col.names = c("Impurity Threshold", "Splits", "Leaves", "Total Nodes", "Accuracy"),
    align = "lcccc",
    escape = FALSE,
    caption = paste0(
...
knitr::include_graphics(path = "figures/tour-none.gif")
knitr::include_graphics(path = "figures/tour-balanced.gif")
knitr::include_graphics(path = "figures/tour-custom.gif")
```
...
# Load F1 results
svmodt_weights_none_f1 <- readRDS("analysis/svmodt_weights_none_f1.rds")
svmodt_weights_balanced_f1 <- readRDS("analysis/svmodt_weights_balanced_f1.rds")
py_stree <- readRDS("analysis/py_stree_f1.rds")
class_imbalance_ratio <- readRDS("analysis/class-imbalance-ratio.RDS")
...
Having outlined the principal components of SVMODT, we now present the complete training procedure. The preceding sections described each mechanism individually, including adaptive feature subsetting, feature selection strategies, pruning controls, feature penalisation, and class weighting. `r knitr::asis_output(ifelse(knitr::is_latex_output(), 'Algorithm \\@ref(alg:svmodt)', 'Figure \\@ref(fig:alg-svmodt-html)'))` integrates these components into a unified recursive tree-building framework.
...
```{r alg-svmodt-html, echo=FALSE, results='asis', eval=knitr::is_html_output(), fig.cap='SVMODT: SVM-based Oblique Decision Tree with Enhanced Splitting', out.width='120%'}
knitr::include_graphics("figures/algo-4.png")
```
...
```{r workflow-install, eval = FALSE, echo = TRUE}
install.packages("svmodt")
library(svmodt)
```

The core function, `svm_split()`, fits an SVMODT model to a data frame. At minimum, the user supplies the training data and the name of the response column; all other arguments (maximum depth, minimum node size, feature selection strategy, class weighting, and feature penalisation) have defaults and can be adjusted as needed.

```{r workflow-fit, echo = TRUE}
...
The fitted object `fit` is a recursively structured tree: each internal node stores the fitted SVM defining its split, and each terminal node stores the assigned class label. Predictions on new data are obtained with the standard `predict()` generic, which recursively applies each node's decision rule until a leaf is reached:

```{r workflow-predict, echo = TRUE}
preds <- predict(fit, penguins_test)
mean(preds == penguins_test$species)
```

Additionally, `trace_path()` maps the path traversed by a single row of data along the tree, reporting the sequence of node-level splits evaluated en route to the final predicted class ensuring that the sequence of splits leading to any individual prediction remains fully transparent and auditable.

```{r workflow-trace, echo = TRUE}
trace_path(object = fit, sample_data = penguins_test, sample_idx = 2)
```

Finally, `plot` visualises the resulting decision boundaries over any two features of interest, overlaying the training observations (see Figure \@ref(fig:workflow-plot)):

```{r workflow-plot, fig.align='center', echo = TRUE, fig.cap = "Decision surface of a fitted SVMODT model on the Palmer Penguins dataset.", out.width = "60%"}
plot(fit, data = penguins_train, response = "species")
```

# Experiments

We evaluate the predictive performance of SVMODT against a range of competing decision tree and support vector machine-based classification methods. These benchmarks include Classification and Regression Trees (CART) using \CRANpkg{rpart}, linear support vector machines using \CRANpkg{e1071}, logistic regression using \CRANpkg{nnet} [@nnet2025], STree, and Accelerated Oblique Random Survival Forests using \CRANpkg{aorsf}. Collectively, these comparators provide a broad reference set spanning conventional axis-parallel trees, linear margin-based classifiers, and alternative oblique tree-based approaches.
...
benchmark_summary_table <- readRDS("analysis/benchmark_summary_table.rds")
...
```{r alg-stree-html, echo=FALSE, results='asis', eval=knitr::is_html_output(), fig.cap='STree (Multi-class SVM-based oblique decision tree)', out.width='120%'}
knitr::include_graphics("figures/algo-5.png")
```
...
best_param <- readRDS(file = "analysis/grid_search_best_params.rds")
