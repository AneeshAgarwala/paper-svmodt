...
SVMODT* remains strongly competitive across the benchmark suite, achieving the highest standalone accuracy on \textbf{Echocardiogram} (0.854) and \textbf{Dermatology} (97.9) while matching the best result on \textbf{Iris} (0.971) and \textbf{Fertility}. It also performs well on several other datasets, including \textbf{Wine} (0.979), \textbf{WDBC} (0.964), and \textbf{Cardiotocography-10} (0.795). SVMODT* relies exclusively on linear support vector machine splits at each node, whereas several competing methods including STree* and AORSF benefit from more flexible non-linear or ensemble-based decision boundaries.
...
Across nine benchmark datasets, \textbf{SVMODT} with tuned hyperparameters (SVMODT*) consistently outperformed conventional axis-aligned CART and remained competitive with the more flexible STree* implementation despite relying exclusively on linear node-wise hyperplanes. On Echocardiogram (0.854), Dermatology (0.979), Iris (0.971), and Fertility, it achieved the strongest or joint-strongest predictive performance among all methods considered. The untuned default configuration served as a robust baseline throughout, indicating that the proposed extensions deliver structural benefits independent of careful hyperparameter search.
