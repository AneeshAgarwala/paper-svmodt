...
We illustrate the typical workflow for fitting, inspecting, and predicting with an SVMODT model using the Palmer Penguins dataset.
...
The core function, \texttt{svm\_split()}, fits an SVMODT model to a data frame. At minimum, the user supplies the training data and the name of the response column; all other arguments (maximum depth, minimum node size, feature selection strategy, class weighting, and feature penalisation) have defaults and can be adjusted as needed.
...
The fitted object \texttt{fit} is a recursively structured tree: each internal node stores the fitted SVM defining its split, and each terminal node stores the assigned class label. Predictions on new data are obtained with the standard \texttt{predict()} generic, which recursively applies each node's decision rule until a leaf is reached:

\begin{verbatim}
preds <- predict(fit, penguins_test)
mean(preds == penguins_test$species)
\end{verbatim}
...
Additionally, \texttt{trace\_path()} maps the path traversed by a single row of data along the tree, reporting the sequence of node-level splits evaluated en route to the final predicted class ensuring that the sequence of splits leading to any individual prediction remains fully transparent and auditable.

\begin{verbatim}
trace_path(object = fit, sample_data = penguins_test, sample_idx = 2)
\end{verbatim}
...
#> Sample 2 :
#>    species = 1 
#>    bill_length_mm = -1.122961 
#>    bill_depth_mm = 0.07547549 
#>    flipper_length_mm = -1.489566 
#>    body_mass_g = -0.6265811 
#> 
#>  [Node 1 ] features = bill_length_mm,bill_depth_mm,flipper_length_mm,body_mass_g 
#>    SVM decision value: 2.7801 
#>    -> Going LEFT (decision > 0)
#>    [Node 2 ] features = bill_length_mm,bill_depth_mm,flipper_length_mm,body_mass_g 
#>      SVM decision value: 2.5264 
#>      -> Going LEFT (decision > 0)
#>      [FINAL] Predict Adelie (n = 121 )
#>      Path taken: LEFT -> LEFT 
#> 
#> Final prediction: Adelie
\end{verbatim}

Finally, \texttt{plot} visualises the resulting decision boundaries over any two features of interest, overlaying the training observations (see Figure \ref{fig:workflow-plot}):

\begin{verbatim}
plot(fit, data = penguins_train, response = "species")
\end{verbatim}

\begin{figure}

{\centering \includegraphics[width=0.6\linewidth]{paper-svmodt_files/figure-latex/workflow-plot-1} 

}

\caption{Decision surface of a fitted SVMODT model on the Palmer Penguins dataset.}\label{fig:workflow-plot}
\end{figure}

\section{Experiments}\label{experiments}

We evaluate the predictive performance of SVMODT against a range of competing decision tree and support vector machine-based classification methods. These benchmarks include Classification and Regression Trees (CART) using \CRANpkg{rpart}, linear support vector machines using \CRANpkg{e1071}, logistic regression using \CRANpkg{nnet} \citep{nnet2025}, STree, and Accelerated Oblique Random Survival Forests using \CRANpkg{aorsf}. Collectively, these comparators provide a broad reference set spanning conventional axis-parallel trees, linear margin-based classifiers, and alternative oblique tree-based approaches.
...
%
%
\href{mailto:aneesh.agarwal@monash.edu}{\nolinkurl{aneesh.agarwal@monash.edu}}%
}
...
%
%
\textit{ORCID: \href{https://orcid.org/0000-0003-2703-7180}{0000-0003-2703-7180}}\\%
...
%
%
\textit{ORCID: \href{https://orcid.org/0000-0001-6093-1390}{0000-0001-6093-1390}}\\%
