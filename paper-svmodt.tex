...
\paragraph{\texorpdfstring{\href{https://github.com/AneeshAgarwala/svmodt}{Github Repository}}{Github Repository}}\label{github-repository}

\section{Introduction}\label{introduction}

Decision trees remain among the most widely adopted machine learning algorithms due to their interpretability, ease of use, and ability to handle both categorical and numerical data without extensive preprocessing. Classical algorithms such as CART \citep{breiman1984} and C4.5 \citep{quinlan1993} employ axis-aligned (univariate) splits that partition the feature space along coordinate axes. While this approach ensures transparency in decision-making, it often necessitates deep trees with many nodes to approximate diagonal or curved decision boundaries, leading to increased model complexity and potential overfitting \citep{pagallo1990}.

\textbf{Oblique decision trees}, which allow splits based on linear combinations of features, address this limitation by enabling more flexible partitioning of the feature space. A single oblique split such as \(w_1x_1 + w_2x_2 < \theta\) can replace multiple axis-aligned splits (\(x_1 < \theta\)) needed to approximate the same boundary. Despite their representational advantages, oblique trees have seen limited adoption in practice, partly due to the computational complexity of finding optimal multivariate splits \citep{brodley1995} and the scarcity of accessible, well-documented software implementations, particularly in R.

Support Vector Machines (SVMs), introduced by \citet{cortes1995}, provide a principled approach to finding optimal separating hyperplanes by maximizing the margin between classes. The integration of SVMs into decision tree structures was pioneered by \citet{bennet1998}, who proposed fitting an SVM at each internal node to determine multivariate splits. This hybrid approach combines the interpretability of trees with the robust margin-based optimization of SVMs. Subsequent work has extended this concept through various algorithmic refinements \citep{takahashi2002, bala2011, ganaie2022}, yet fully-featured, accessible implementations remain scarce in the R ecosystem.

\subsection{Research Objectives}\label{research-objectives}

This paper addresses three key limitations in existing decision tree implementations:

\textbf{First}, we tackle the computational inefficiency of traditional two-phase pruning approaches. Classical methods like CART grow a maximal tree before applying cost-complexity pruning via cross-validation \citep{breiman1984}. This approach wastes computation on branches that are ultimately removed and requires tuning multiple hyper-parameters. We propose \textbf{integrated cost-complexity pruning}, where pruning decisions are made in real-time during tree construction. We propose integrated cost-complexity pruning where pruning decisions are made in real-time during tree construction to maintain theoretical optimality.

\textbf{Second}, we address the lack of flexible oblique tree implementations in R. While Python's STree \citep{Montanana2021} and specialized packages exist, R practitioners lack access to SVM-based oblique trees with modern features such as dynamic feature selection, node-specific class weighting, and built-in visualization tools. Our \textbf{SVMODT} (SVM-based Oblique Decision Tree) implementation fills this gap with a comprehensive, well-documented package designed for both research and practical applications.
...
A simplest \textbf{linear SVMs} construct a separating hyperplane in an \(p\)-dimensional space such that the margin between the classes is maximized. Given a training dataset \(\{(\mathbf{x}_i, y_i)\}_{i=1}^{N}\), where \(\mathbf{x}_i \in \mathbb{R}^p\) and \(y_i \in \{-1, +1\}\), the decision function is defined as:
...
he constrained optimization problem can be efficiently solved using standard quadratic programming solvers, such as those available in the \CRANpkg{quadprog} package in R. The data points corresponding to non-zero \(\alpha_i\) values are the \textbf{support vectors}, which define the decision boundary. The resulting decision function for a new observation \(\mathbf{x}\) is given by:
...
\subsection{Limited Flexibility in Oblique Decision Tree Software}\label{limited-flexibility-in-oblique-decision-tree-software}
...
\item
  \textbf{interpretability tools}: Despite the advantage of tree-based models in interpret ability, existing packages lack comprehensive visualization tools for examining individual SVM hyper-planes, tracing prediction paths, or understanding how oblique splits contribute to final decisions.
\end{itemize}

\subsection{Absence of Integrated Pruning in Modern Decision Trees}\label{absence-of-integrated-pruning-in-modern-decision-trees}
...
\item
  \textbf{Select Optimal Split}: At each node, the algorithm evaluates all features and every possible split point \(n-1\) for \(n\) samplesl to identify the split that maximizes class separation according to the chosen criterion:
...
\item
  \textbf{response}: Name of the response variable (as a string)
\item
  \textbf{max\_depth}: Maximum tree depth (controls model complexity)
\item
...
Terminal nodes (labeled as \emph{Leaf}) correspond to final class predictions. In this case, most observations in the left sub-tree are classified as \emph{Adelie}, while the right sub-tree primarily identifies \emph{Chinstrap} penguins.
...
Fig \ref{fig:penguins-depth-one} depicts the root node of the SVM-based oblique decision tree, where the first split is performed using the features \texttt{bill\_length\_mm} and \texttt{flipper\_length\_mm}. The figure illustrates how the linear SVM at the root node divides the dataset into two branches, guiding samples toward subsequent child nodes.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Custom Decision Tree} - Axis-aligned decision tree using Gini impurity to select splits, limited to depth 4, with cost-complexity pruning controlled by \(\alpha = 0.01\) to prevent overfitting.
\item
  \textbf{SVMODT} - Oblique decision tree using linear SVMs at each node, selecting features via mutual information, limiting depth to 4, with decreasing feature count (rate 0.5) and penalization to reduce repeated feature usage.
\item
  \textbf{CART (rpart)} - Oblique decision tree using linear SVMs at each node, selecting features via mutual information, limiting depth to 4, with decreasing feature count (rate 0.5) and penalization to reduce repeated feature usage.
\item
  \textbf{Linear SVM} - Linear kernel SVM with default C=1 regularization controlling the margin--error trade-off.
\item
  \textbf{RBF SVM} - Radial Basis Function kernel SVM with default C=1 regularization and sigma estimated from the data, controlling kernel smoothness.
\item
  \textbf{Logistic Regression} - Linear model for binary classification with L2 regularization (ridge penalty) to prevent overfitting by shrinking coefficients.
\end{enumerate}
...
\hline
SVMODT & 0.9737 & 0.0146 & 0.9304 & \textbf{1.0000}\\
\hline
RPART & 0.9289 & 0.0214 & 0.8609 & 0.9652\\
\hline
Linear SVM & 0.9730 & 0.0139 & 0.9304 & \textbf{1.0000}\\
\hline
RBF SVM & \textbf{0.9745} & \textbf{0.0129} & \textbf{0.9391} & \textbf{1.0000}\\
\hline
Logistic Regression & 0.9450 & 0.0215 & 0.8783 & \textbf{1.0000}\\
\hline
\end{tabular}
\end{table}
...
From Table \ref{tab:t-test-results-latex}, positive mean differences indicate that the first model in each comparison outperformed the second. Paired t-tests were used to assess whether differences in test accuracy across repeated train--test splits are statistically significant. This test assumes that the differences between paired observations are approximately normally distributed and that the pairs are independent.

The \textbf{Custom Tree} slightly outperformed \texttt{rpart} (mean difference = 0.0034, p = 0.0362), indicating a marginal but significant improvement at \(\alpha = 5\%\). \textbf{SVMODT} performed comparably to \textbf{Linear SVM} (mean difference = 0.0006, p = 0.2880) and \textbf{RBF SVM} (mean difference = -0.0009, p = 0.4801), but significantly outperformed the Custom Tree (mean difference = 0.0414, p \textless{} 0.001) and \textbf{Logistic Regression} (mean difference = 0.0287, p \textless{} 0.001). Conversely, the Custom Tree performed worse than Logistic Regression (mean difference = -0.0127, p \textless{} 0.001).
