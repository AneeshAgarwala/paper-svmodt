...
# ── Parameter Grid ─────────────────────────────────────────────────────────────
# min_samples: removed — fixed at default in svm_split
...
param_grid_base <- expand.grid(
  ...
  min_impurity_decrease  = c(0.0, 0.01, 0.05),
...
# ── Build Args ─────────────────────────────────────────────────────────────────
# min_samples removed — not a tuned parameter
# impurity_measure fixed at "entropy"
...
# ── Custom caret Model ─────────────────────────────────────────────────────────
# min_samples and impurity_measure removed from parameters
svmodt_caret_model <- list(
  ...
  grid = function(x, y, len = NULL, search = "grid") {
    param_grid_base |> mutate(max_features = floor(sqrt(ncol(x))))
  },
  ...
  prob = NULL,
  sort = function(x) x[order(x$max_depth), ]
)
...
# ── Rerun 10 × 5-fold CV with best params ─────────────────────────────────────
...
for (dataset_name in names(datasets)) {
  ...
  bp <- best_params |> filter(dataset == dataset_name) |> slice(1)
  ...
  best_args <- list(
    ...
    max_depth              = if (has_col("max_depth"))             bp_num("max_depth")             else 15,
    max_features_strategy  = strategy,
    penalize_used_features = penalize,
    impurity_measure       = "entropy",   # fixed
  ...
  # sanity_folds <- vfold_cv(datasets[[dataset_name]], v = 5, strata = clase)
  # 
  # sanity_ok <- tryCatch({
  #   m <- do.call(svmodt::svm_split,
  #                c(list(data     = analysis(sanity_folds$splits[[1]]),
  #                       response = "clase"), best_args))
  #   p   <- svm_predict_tree(m, assessment(sanity_folds$splits[[1]]))
  #   acc <- mean(p == assessment(sanity_folds$splits[[1]])$clase, na.rm = TRUE)
  #   cat("  Sanity check acc     :", round(acc, 4), "\n")
  #   TRUE
  # }, error = function(e) {
  #   message("  [ERROR] sanity check failed: ", conditionMessage(e))
  #   FALSE
  # })
  # 
  # if (!sanity_ok) {
  #   r_svmodt_star_results[[dataset_name]] <- rep(NA_real_, 10)
  #   next
  # }
  ...
  for (i in 1:10) {
    ...
    fold_accs[i] <- mean(map_dbl(folds$splits, function(split) {
      ...
      tryCatch({
        model <- do.call(svmodt::svm_split,
                         c(list(data = train_data, response = "clase"), best_args))
        preds <- svm_predict_tree(model, test_data)
        mean(preds == test_data$clase, na.rm = TRUE)
      }, error = function(e) {
        message("  [WARN] ", dataset_name, " seed ", seed_list[i],
                ": ", conditionMessage(e))
        NA_real_
      })
    }), na.rm = TRUE)
