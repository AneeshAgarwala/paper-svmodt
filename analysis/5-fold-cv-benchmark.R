...
make_train_svmodt <- function(extra_args) {
  function(data, response) {
    do.call(
      svmodt::svm_split,
      c(list(data = data, response = response),
        base_svmodt_args,
        extra_args)   # extra_args override base where they overlap
    )
  }
}
        ...
        fold_accs <- map_dfc(names(variants), function(variant_name) {
          ...
          tryCatch({
            ...
            if (compute_f1) {
              ...
              tibble(!!paste0(variant_name, "_acc") := acc,
            ...
            message("  [WARN] ", variant_name, " - ", dataset_name,
                    ": ", conditionMessage(e))
            if (compute_f1) {
              tibble(!!paste0(variant_name, "_acc") := NA_real_,
  ...
  map(results, function(res) {
    ...
    if (length(unique(entry_lengths)) > 1) {
      warning(
        ...
        " — padding shorter entries with NA"
      )
...
r_stree  <- do.call(cbind, results_bench$r_stree);  colnames(r_stree)  <- dataset_col_names
r_svmodt <- do.call(cbind, results_bench$r_svmodt); colnames(r_svmodt) <- dataset_col_names
py_stree <- do.call(cbind, results_bench$py_stree); colnames(py_stree) <- dataset_col_names
...
# Convert to matrices
r_stree_f1  <- do.call(cbind, results_bench_f1$r_stree);  colnames(r_stree_f1)  <- dataset_col_names
r_svmodt_f1 <- do.call(cbind, results_bench_f1$r_svmodt); colnames(r_svmodt_f1) <- dataset_col_names
py_stree_f1 <- do.call(cbind, results_bench_f1$py_stree); colnames(py_stree_f1) <- dataset_col_names
...
penalise_variants <- list(
  ...
  penalty_low    = list(feature_method = "mutual", max_features = 4, penalize_used_features = TRUE,
                        feature_penalty_weight = 0.2),
  penalty_medium = list(feature_method = "mutual", max_features = 4, penalize_used_features = TRUE,
                        feature_penalty_weight = 0.5),
  penalty_high   = list(feature_method = "mutual", max_features = 4, penalize_used_features = TRUE,
